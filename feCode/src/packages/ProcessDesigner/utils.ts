// 创建监听器实例
export function createListenerObject(bpmnInstances, options, isTask, prefix) {
  const listenerObj = Object.create(null)
  listenerObj.event = options.event
  // isTask && (listenerObj.id = options.id); // 任务监听器特有的 id 字段
  switch (options.listenerType) {
    case 'scriptListener':
      listenerObj.script = createScriptObject(bpmnInstances, options, prefix)
      break
  ...
  if (options.fields) {
    listenerObj.fields = options.fields.map((field) => {
      return createFieldObject(bpmnInstances, field, prefix)
    })
  }
  // 任务监听器的 定时器 设置
...
// 创建脚本实例
export function createScriptObject(bpmnInstances, options, prefix) {
...
// 更新元素扩展属性
export function updateElementExtensions(bpmnInstances, element, extensionList) {
