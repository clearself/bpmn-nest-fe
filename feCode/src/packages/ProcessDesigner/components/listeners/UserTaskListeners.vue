<template>
  ...
  <a-drawer v-model:open="listenerFormModelVisible" title="任务监听器" width="500px" placement="right" destroyOnClose>
    <a-form :model="listenerForm" layout="vertical" ref="listenerFormRef">
      ...
      <template v-if="listenerForm.event === 'timeout'">
        ...
        >
          <a-input v-model:value.trim="listenerForm.eventTimeDefinitions" :placeholder="$t('inputTips')" allowClear />
        </a-form-item>
      </template>
    </a-form>
    <a-divider />
...
import { initListenerType, initListenerForm, listenerType, eventType, fieldType } from './utilSelf'
// 使用inject注入函数
...
const listenerEventTypeObject: any = ref(eventType)
...
const resetData = () => {
  ...
  bpmnElementListeners.value =
    props.businessObject?.extensionElements?.values?.filter((ex: any) => ex.$type === `${prefix}:TaskListener`) ?? []
...
const saveListenerConfig = async () => {
  ...
  const listenerObject = createListenerObject(bpmnInstances, listenerForm.value, true, prefix)
