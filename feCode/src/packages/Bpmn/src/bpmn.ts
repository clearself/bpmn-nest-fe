/*
 * @Author: Fu leiyang
 * @Date: 2024-07-15 16:24:48
 * @LastEditors: Fu leiyang
 * @Description: 描述
 */
  ...
  initModeler(options: any) {
    ...
    EVENT_TYPE.forEach((event) => {
      this.addEventListener(event, (elementAction: any) => {
        ...
        const elementTypeNonEqualActive = this?.activeElement?.element && element.id !== this.activeElement.element.id
        ...
        nextTick().then(() => {
          ...
          if (
            ...
            const length =
              this.getShapeAll().filter((item) => ['bpmn:StartEvent', 'bpmn:UserTask'].includes(item.type)).length || 0
            this.updateProperties(element, {
              name: element.type == 'bpmn:StartEvent' ? '开始节点' : '未命名节点' + length
            })
          }
          console.log(elementAction, 'elementActionelementActionelementActionelementAction')
