  ...
  <div class="search-wrapper">
    <a-card size="small">
      <a-form :model="searchParams" layout="inline">
        <a-form-item :label="$t('post.columns.postCode')">
          <a-input
            allowClear
            v-model:value.trim="searchParams.postCode"
            :placeholder="$t('inputTips')"
            style="width: 220px"
          />
        </a-form-item>
        <a-form-item :label="$t('post.columns.postName')">
          <a-input
            allowClear
            v-model:value.trim="searchParams.postName"
            :placeholder="$t('inputTips')"
            style="width: 220px"
          />
        </a-form-item>
  ...
  <div class="ub ub-ac ub-pj top-opt w100">
    ...
    <div>
      <a-button type="link" ref="ref2" shape="circle">
        <fields-setting tabKey="post-table" v-model:value="columnsList" :columns="columns" />
      </a-button>
    </div>
  </div>
  <a-card size="small">
    <div class="mt-1 w100" id="postTable">
      <a-table
...
const toDelete = (record: any) => {
  ...
  return new Promise(async (resolve) => {
    const res: ApiResponseData = await deleteItem({ ids: record.postId })
    if (res?.code === 200) {
