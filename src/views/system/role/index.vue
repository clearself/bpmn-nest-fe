<template>
  <div class="search-wrapper">
    <a-card size="small">
      <a-form :model="searchParams" layout="inline">
        <a-form-item :label="$t('role.columns.roleName')">
          <a-input
            allowClear
            v-model:value.trim="searchParams.roleName"
            :placeholder="$t('inputTips')"
            style="width: 220px"
          />
        </a-form-item>
        <a-form-item :label="$t('role.columns.roleKey')">
          <a-input
            allowClear
            v-model:value.trim="searchParams.roleKey"
            :placeholder="$t('inputTips')"
            style="width: 220px"
          />
        </a-form-item>
  ...
  <div class="ub ub-ac ub-pj top-opt w100">
    ...
    <div>
      <a-button type="link" ref="ref2" shape="circle">
        <fields-setting tabKey="role-table" v-model:value="columnsList" :columns="columns" />
      </a-button>
    </div>
  </div>
  <a-card size="small">
    <div class="mt-1 w100" id="roleTable">
      <a-table
      ...
      >
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-if="column.dataIndex === 'operation'">
            ...
            <a-button type="link" size="small" v-if="record.roleId !== 1" @click="showDataBox(record)">{{
              $t('role.opts.dataPermiss')
            }}</a-button>
            <!-- <a-button type="link" size="small" v-if="record.roleId !== 1">{{
              $t('role.opts.assigningUsers')
            }}</a-button> -->
            <a-popconfirm
              :title="$t('sureTips')"
              :ok-text="$t('opts.sure')"
  ...
  <form-data-model v-model:show="formDataShow" :cur-data="curData" @success="onSuccess" />
</template>
...
const toDelete = (record: any) => {
  ...
  return new Promise(async (resolve) => {
    const res: ApiResponseData = await deleteItem({ id: record.roleId })
    if (res?.code === 200) {
...
const showDataBox = async (record: any) => {
  const res = await getRole(record.roleId)
  if (res?.code === 200) {
    curData = res.data
    console.log(res.data, 'res.datares.data')
    formDataShow.value = true
  }
}
